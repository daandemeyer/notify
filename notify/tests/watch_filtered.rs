...
use notify::{
    recommended_watcher, Config, ErrorKind, Event, PathOp, PollWatcher, RecursiveMode, Result,
    WatchFilter, WatchPathConfig, Watcher,
};
...
#[test]
fn overlapping_filtered_watches_are_refused_recommended() {
    let dir = testdir();
    let root = dir.path();
    let (tx, _rx) = mpsc::channel();
    let mut watcher = recommended_watcher(tx).expect("recommended watcher");
    ...
    let error = match result {
        Err(error) => error,
        Ok(()) => panic!("a filtered watch overlapping another must be refused"),
    };
    assert!(
        matches!(error.kind, ErrorKind::WatchOverlap),
        "expected WatchOverlap, got {error:?}"
    );
    assert_eq!(
        error.paths,
        vec![child.clone(), root.to_path_buf()],
        "the error must name the requested path and the one it conflicts with"
    );
    assert_eq!(
        watcher.watched_paths().expect("watched paths"),
        before,
        "a refused watch must leave the existing watches unchanged"
    );
}

/// `update_paths` has to carry the per-watch filter all the way to the backend. This is the
/// only public entry point that does, so exercise it end to end rather than through
/// `watch_filtered`.
#[test]
fn update_paths_applies_the_watch_filter() {
    let dir = testdir();
    let root = dir.path();
    ...
    assert!(
        ...
        "the watch installed by update_paths must be live: {events:?}"
    );
    assert!(
        events.iter().all(|e| e
            .paths
        ...
        "update_paths must carry the filter through to the backend: {events:?}"
    );
}

/// A rejected root must fail the whole batch and register nothing.
#[test]
fn update_paths_rejected_root_is_path_excluded() {
    let dir = testdir();
    let root = dir.path();
    let rejecting = root.to_path_buf();

    let (tx, _rx) = mpsc::channel();
    let mut watcher = PollWatcher::new(tx, Config::default()).expect("poll watcher");
    let result = watcher.update_paths(vec![PathOp::Watch(
        root.to_path_buf(),
        WatchPathConfig::new(RecursiveMode::Recursive).with_watch_filter(WatchFilter::with_filter(
            move |p: &Path| p != rejecting.as_path(),
        )),
    )]);

    assert!(
        matches!(result, Err(ref e) if matches!(e.source.kind, ErrorKind::PathExcluded)),
        "update_paths must surface PathExcluded for a rejected root: {result:?}"
    );
    assert!(watcher.watched_paths().expect("watched paths").is_empty());
}
...
fn recommended_watcher_suppresses_events_inside_excluded_directory() {
    ...
    // The loop above stops the moment an included event arrives, so a leaked event could
    // still be queued behind it. Let the backend settle and drain everything that is left,
    // so the no-leak assertion does not depend on delivery order.
    std::thread::sleep(Duration::from_millis(500));
    while let Ok(res) = rx.try_recv() {
        let event = res.expect("watcher delivered an error");
        assert!(
            !event
                .paths
                .iter()
...
/// when the filter would reject that exact path.
#[test]
